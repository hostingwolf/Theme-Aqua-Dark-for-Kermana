...
<form method="post" action="{$site_url}/includes/add_process.php" enctype="multipart/form-data">
    ...
    <div class="panel panel-default form-add">
        ...
        <!--<div class="panel panel-default panel-captcha-add">
            <div class="panel-heading heading-captcha"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span> {$l_captcha}</div>
            <table border="0" width="100%" class="table table-form">
                <tr>
                    <td width="20%">{$l_enter_captcha}</td>
                    <td width="0%"><img src="{$site_url}/visual.php" alt="captcha" /><div style="margin-top:10px"><input type="text" name="visual" required="required" /></div></td>
                </tr>
            </table>
        </div> -->

        <!-- Start new layout captcha -->
        <div class="panel panel-default panel-captcha-add">
            <div class="panel-heading heading-captcha"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span> {$l_captcha}</div>
            <div class="box-captha"><p>{$l_enter_captcha}</p></div>
            <div class="text-center img-captcha"><img src="{$site_url}/visual.php" alt="captcha" /></div>
            <div class="text-center"><input type="text" name="visual" required="required" /></div>               
        </div>
        <!-- End new layout captcha -->
        ...
        <div class="text-center">
            ...
            <!-- ENDIF -->
            <button type="submit" class="btn btn-primary bt-submit-add">{$l_submit}</button>
        </div>
</form>
...
<script>
    function confirm_remove()
    {
        ...
        if (c)
            document.location = "{$site_url}/add.php?cmd=del&item_id={$item_id}&AXSRF_token={$axsrf}";
                }
...
// (c) Larry Battle <bateru.com/news>
                ...
                $('#item_backlink_url').blur(function () {
