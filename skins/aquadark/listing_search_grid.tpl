...
<h1 class="h1-details" style="margin-top:0">{$cat_name}</h1>

<div class="befor-p">{$cat_details}</div>
<!-- ENDIF -->
...
<h2 class="h1-details">{$l_featured_listing}</h2>
...
<h2 class="h1-details">{$l_listing}</h2>

<!-- BEGINIF $no_search_result -->
<div class="foot-search-listing">{$l_search_no_result}</div>
<!-- ELSE -->
<div class="row">
...
<script>
    ...
    $(function () {
        $('#search_filter_main_form select').change(function () {
            search_filter_submit()
        });
        $('#search_filter_main_form input:radio').change(function () {
            search_filter_submit()
        });
        $('#search_filter_main_form input:checkbox').change(function () {
            search_filter_submit()
        });
        $('#search_filter_main_form').submit(function () {
            search_filter_submit()
        });
