...
<script>
    function promptPage()
    {
        var page = prompt('{$l_enter_page_number}: 1-{$pg_total_pages}', '{$pg_current_page}');
                ...
                if (isNaN(pageInt))
                    return false;
                if ((pageInt > {$pg_total_pages}) || (pageInt < 1) || (pageInt == {$pg_current_page}))
                    return false;
...
<script>
    ...
    $('#search_filter_form input:radio').change(function () {
        search_filter_submit()
    });
    $('#search_filter_form input:checkbox').change(function () {
        search_filter_submit()
    });
    $('#search_filter_form input:text').keypress(function (e) {
        if (e.which == 13) {
            search_filter_submit();
            e.preventDefault()
        }
...
<script>
    $(function () {
        function details_in_popup(link, div_id) {
            $.ajax({url: link, success: function(response){ $('#'+div_id).html(response); } });
            return '<div id="' + div_id + '" style="max-height:500px;overflow:auto">Loading...</div>';
        }
        $('#popup-notify').popover({"html": true,"content": function(){var div_id =  "tmp-id-" + $.now();return details_in_popup($(this).attr('data-href'), div_id);}}).on("show.bs.popover", function () {
