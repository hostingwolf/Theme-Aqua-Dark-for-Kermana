...
<ol class="breadcrumb">
    ...
    <h1 class="h1-add">{$l_select_dir}</h1>
<div class="row">
    <!-- BEGINBLOCK list -->
    <div class="col-sm-6">
        <div class="dir_select thumbnail box-thumb">
            <a href="add.php?cmd=add&amp;dir_id={$idx}"><img src="{$dir_image}" alt="{$dir_title}" /></a>
            <div class="caption">
                <h3>{$dir_title}</h3>
                <div class="dir_desc">{$dir_body}</div>
                <p class="bt-div"><a href="add.php?cmd=add&amp;dir_id={$idx}" class="btn btn-primary bt-add" role="button">{$l_select}</a></p>
            </div>
        </div>
    </div>
    <!-- ENDBLOCK -->
