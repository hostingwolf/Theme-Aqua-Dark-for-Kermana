...
<div class="wrap-details">
    <h1 class="h1-details"cla >{$item_title} <small><button class="btn btn-xs btn-default" title="{$visible_help}">{$visible_icon}</button></small></h1>
    ...
    <h2 class="h1-details">{$l_description}</h2>
    <!-- BEGINIF $require_summary --><p>{$item_summary}</p><!-- ENDIF -->
    <p class="p-details">{$item_details}</p>
    ...
    <h2 class="h2-details">{$l_specification}</h2>
</div>
<!-- End wrap -->

<div class="panel panel-default panel-aquadark">
...
<h2 class="h2-details" >{$l_listing_info}</h2>

<div class="list-group list-group-aqua-dark">
    <!-- BEGINIF $edit_btn -->
    ...
    <div class="list-group-item" href="{$site_url}/listing_search.php?owner_id={$owner_id}"><span class="glyphicon glyphicon-user"></span> {$l_posted_by}: {$owner_id} - {$item_date}</div>
    <div class="list-group-item"><span class="glyphicon glyphicon-eye-open"></span> Hits: {$stat_hits}&times;</div>
    <div class="list-group-item"><span class="glyphicon glyphicon-th-list"></span> {$l_posted_on}: {$listed_cat}</div>
</div>

<!-- BEGINIF $enable_comment -->
<h2 class="h2-details">{$l_review}</h2>
